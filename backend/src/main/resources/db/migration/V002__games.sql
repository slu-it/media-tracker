-- Flyway migration V002: games (MT-001). Mirrored by de.sluit.mediatracker.games.persistence.{GamesTable,
...
-- script again once it has shipped; add a new V<nnn> instead.
--
-- id columns hold the UUID in its 36-character hex-dash form. Exposed's uuid() column would be BINARY(16) on
-- MariaDB but UUID on H2 and fail SchemaDriftTest; CHAR(36) is identical on both engines and readable in SQL

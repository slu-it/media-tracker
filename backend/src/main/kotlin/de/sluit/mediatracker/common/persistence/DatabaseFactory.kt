...
object DatabaseFactory {
    ...
    /**
     * Compares the Exposed [tables] with the live schema. A non-empty result means the Kotlin model and the
     ...
     */
    fun warnOnSchemaDrift(database: Database, tables: Array<out Table>) {
        val statements = schemaDriftStatements(database, tables)
    ...
    /** The statements Exposed would need to make the live schema match [tables]; empty when in sync. */
    fun schemaDriftStatements(database: Database, tables: Array<out Table>): List<String> = transaction(database) {

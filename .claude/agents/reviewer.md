...
4. **Database**: routes use `dbQuery { }`, `*Blocking` only inside an existing transaction; applied migrations untouched; `${timestamp_type}`, `CHAR(36)` ids, FK index in SQL and Kotlin; SQL valid on MariaDB 11.8 and H2 MariaDB mode.

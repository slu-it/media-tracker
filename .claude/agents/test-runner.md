...
- Append `--console=plain` to Gradle commands. Inside Claude Code (`CLAUDECODE=1`) `:backend:test` logs only failed and
  skipped tests, so the output is not truncated. Never use Gradle's `--quiet`, which hides failures too. Use the maximum Bash timeout (10 minutes) for anything Gradle. A cold run downloads Node and pnpm and can exceed even that, so start `./gradlew build` with the Bash tool's own `run_in_background: true` parameter and then stop: the harness re-invokes you with the exit code when it finishes. Never run two Gradle commands concurrently.
- Never wait by polling in the shell. A loop like `while kill -0 $(pgrep -f "gradlew build"); do sleep 5; done` never ends, because `pgrep -f` matches the shell command that contains that very pattern - the loop waits on itself. Waiting is the harness's job, not a command's. If you genuinely must match a process, exclude your own command with a bracket pattern (`pgrep -f '[g]radlew build'`) and never let a match decide whether you keep waiting.

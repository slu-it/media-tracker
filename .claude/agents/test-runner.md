...
- Append `--console=plain` to Gradle commands. Use the maximum Bash timeout (10 minutes) for anything Gradle. A cold run downloads Node and pnpm and can exceed even that, so start `./gradlew build` with the Bash tool's own `run_in_background: true` parameter and then stop: the harness re-invokes you with the exit code when it finishes. Never run two Gradle commands concurrently.
